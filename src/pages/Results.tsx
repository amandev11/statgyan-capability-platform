...
function ResultsBody({
  ...
  // AI competency insight — explains the REAL evidence via the server-side
  // provider; the deterministic sentence below remains the honest fallback so
  // this section never blanks when AI is unavailable.
  const fetchInsight = useAction(api.ai.competencyInsight);
  const [aiInsight, setAiInsight] = useState<string | null>(null);
  useEffect(() => {
    if (!profile || impact.length === 0) return;
    let live = true;
    const gaps = analyseGaps(profile.competencies, {
      primaryDomain: profile.primaryDomain,
      secondaryDomains: profile.secondaryDomains,
    });
    const strengths = [...profile.competencies].sort((a, b) => b.score - a.score).slice(0, 2);
    void fetchInsight({
      gaps: gaps
        .slice(0, 3)
        .map((g) => ({ name: g.name, score: g.current, target: g.target, gap: g.gap })),
      strengths: strengths.map((c) => ({ name: domainName(c.id), score: c.score })),
      recentEvidence: impact.map((d) => `${d.label} ${d.correct}/${d.asked}`).join("; "),
      recommendationTitle: gaps[0]?.name,
    })
      .then((r) => {
        if (live) setAiInsight(r.insight);
      })
      .catch(() => {});
    return () => {
      live = false;
    ...
    <PageContainer width="reading">
      ...
      >
        ...
        <div className="edge-glow mt-6 rounded-2xl border hairline bg-[var(--qz-surface-1)] p-6">
          ...
          <p className="mt-2 text-[11px] text-muted-qz">
            {aiInsight
              ? "Generated by the StatGyan AI engine from your assessment evidence"
              : "Computed deterministically from your assessment evidence"}
          </p>
